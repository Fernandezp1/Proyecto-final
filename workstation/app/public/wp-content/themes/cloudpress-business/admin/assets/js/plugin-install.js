jQuery(document).ready(function($) {
    'use strict';
    var this_obj = cloudpress_business_companion_install;
    ...
    $(document).on('click', '.action-watch', function(event) {
        ...
        if(action_id){
            $.ajax({
                ...
                type: 'POST',
                data: {action: 'cloudpress_business_update_rec_acts', action_id:action_id},
                success:function(data) {
    ...
    $(document).on('click', '.spicethemes-customizer-notification-dismiss', function(event) {
        ...
        var sdata = {};
        if(slug){
            sdata.action = 'cloudpress_business_hide_customizer_notice';
            sdata.cloudpress_business_plugin_slug = slug;
        }else{
            sdata.action = 'cloudpress_business_hide_customizer_companion_notice';
        }
